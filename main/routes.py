...
def get_channel_messages():
    channel_data = json.loads(requests.get(
        'https://slack.com/api/channels.history?{slack_token}&channel={channel_id}&latest={latest}&count=5&pretty=1'.format(
            slack_token=os.environ['slack_token'],channel_id=os.environ['channel_id'],latest=time.time())).text)
    channel_messages = {}
    j = 0
